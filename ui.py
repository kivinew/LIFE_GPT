...
class Slider:
    ...
    def draw(self, surf, font):
        if self.unit:
            txt = font.render(f"{self.lab}: {self.val:.0f}{self.unit}", True, WHITE)
        else:
            txt = font.render(f"{self.lab}: {self.val:.2f}", True, WHITE)
        surf.blit(txt, (self.rect.x, self.rect.y - 12))
        pygame.draw.rect(
...
class SliderInt:
    ...
    def draw(self, surf, font):
        ...
        txt = font.render(f"{self.lab}: {current_label}", True, WHITE)
        surf.blit(txt, (self.rect.x, self.rect.y - 12))
        pygame.draw.rect(
            surf, DARK, (self.rect.x, self.rect.y + 3, self.rect.w, 6), 0, 3
        )
...
class Toggle:
    ...
    def draw(self, surf, font):
        ...
        txt = font.render(self.labels[self.val], True, WHITE)
        surf.blit(
            txt,
            (
...
class PopulationGraph:
    ...
    def draw(self, surf, x, y, width, height):
        ...
        pygame.draw.rect(surf, DARK, bg_rect)
        ...
        for k in range(start_k, end_k + 1):
            ...
            if bx1 <= bx0:
                continue
            band.fill((*_SEASON_COLOR[season], 40), (bx0 - x, 0, bx1 - bx0, height))
            if bx1 - bx0 >= 40:
                font = pygame.font.SysFont(None, 14)
                label = font.render(tr(season), True, WHITE)
                band.blit(label, (bx0 - x + 3, 3))
        surf.blit(band, (x, y))
        pygame.draw.rect(surf, GRAY, bg_rect, 1)
        ...
        def scale_y(val):
            return (
                y + height - (val / max_count) * (height - 10)
            )  # leave 5px margin top/bottom
        ...
        # X axis
        pygame.draw.line(
            surf, GRAY, (x + 5, y + height - 5), (x + width - 5, y + height - 5), 1
        )
        # Y axis
        pygame.draw.line(surf, GRAY, (x + 5, y + 5), (x + 5, y + height - 5), 1)

        # Optional: draw ticks on Y axis
        for i in range(1, 5):
            val = (i / 4) * max_count
            py = y + height - (i / 4) * (height - 10)
            pygame.draw.line(surf, GRAY, (x + 5, py), (x + 8, py), 1)
            font = pygame.font.SysFont(None, 14)
            text = font.render(f"{int(val)}", True, WHITE)
            surf.blit(text, (x - 30, py - 7))
